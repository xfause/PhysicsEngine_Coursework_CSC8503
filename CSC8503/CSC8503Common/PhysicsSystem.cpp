	...
	for (auto i = first; i != last; i++) {
		...
		for (auto j = i + 1; j != last; j++) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				if ((info.a)->GetPhysicsObject()->GetUseSpring() || (info.b)->GetPhysicsObject()->GetUseSpring()) {
...
// use spring model to simulate collision responde
void PhysicsSystem::ResolveSpringCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& point) const
{
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();

	Spring s(0.0f, 0.5f);
	s.SetDamping(1.0 - physA->GetLinearDamping() * physB->GetLinearDamping());

	Vector3 force = point.normal * -(s.GetSpringConstant() * (point.penetration - s.GetLength()));
	force *= s.GetDamping();
